...
namespace MTSICS_DAO
{
    public class EY_Batch_DAO
    {
        ...
        public static IList<EY_Batch> LoadAll()
        {
            ...
            using (ISession session = SessionHelper.GetSession())
            {
                IQuery query = session.CreateQuery("from EY_Batch as a");
        ...
        public static IList create4Excel(DateTime less, DateTime great)
        {
            ...
            using (ISession session = SessionHelper.GetSession())
            {
                IQuery query = session.CreateQuery("from EY_Batch as a ,RegNo_DI as b where a.CreateTime>=:less and a.CreateTime<=:great and a.BatchNo=b.Batch");
                query.SetDateTime("less", less);
                query.SetDateTime("great", great);
                ret = query.List();
            }
        ...
        public static String GetBatchNo()
        {
            ...
            using (ISession session = SessionHelper.GetSession())
            {
                IQuery query = session.CreateQuery("select max(a.BatchNo) from EY_Batch as a");
